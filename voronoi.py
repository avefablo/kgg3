...
from data_types import Event, PriorityQueue, Segment, Arc, Point
...
class Voronoi:
    def __init__(self, points):
        ...
        self.y0 = 550.0
        self.y1 = 550.0
        points = [e for e in sorted(points, key=lambda e: e.y)]
    ...
    def process(self):
        while not self.points.empty():
            if not self.event.empty() and \
                            self.event.top().x <= self.points.top().x:
    ...
    def intersect(self, p, i):
        ...
        if (((i.pprev is None) or (a <= p.y)) and (
            ...
            py = p.y
            px = 1.0 * (i.p.x ** 2 + (i.p.y - py) ** 2 - p.x ** 2) / (
                2 * i.p.x - 2 * p.x)
            res = Point(px, py)
            return True, res
        return False, None
